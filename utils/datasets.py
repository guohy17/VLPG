...
def img2cls_paths(img_paths):
    # Define label paths as a function of image paths
    a = [("/data3/hyguo/CROHME_labels/CROHME_train/BBOX/" + x.split("/")[-1]).replace("jpg", "txt") for x in img_paths]
    return a
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        # self.label_files = img2cls_paths(self.img_files)
        # cache_path = Path(self.label_files[0]).parent.with_suffix('.cache')  # cached labels
        cache_path = Path("./train.cache")
        # cache_path = Path("/lustre/home/chuang/share/datasets/math_formula/crohme/train.cache")
        ...
        # self.label_files = img2cls_paths(cache.keys)
        if single_cls:
